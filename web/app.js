...
$('generateBtn').onclick=async()=>{
  ...
  try{
    ...
    const audioUrl=`data:${body.mime_type||'audio/wav'};base64,${body.audio_base64}`; $('audio').src=audioUrl; $('download').href=audioUrl; $('download').download=`laxman-lofi-${Date.now()}.wav`; $('resultTitle').textContent=$('songTitle').value.trim()||'Your generation'; $('result').classList.remove('hidden'); $('seed').textContent=body.seed??'—'; $('metaDuration').textContent=(body.duration||$('duration').value)+' sec'; $('result').scrollIntoView({behavior:'smooth'});
  }catch(err){alert(err.message||String(err));}
  finally{btn.disabled=false;$('progress').classList.add('hidden');}
};
